"""Retraining-based PRISM-VQ backbone ablations with a TA-PRISM adapter.

Each run trains and evaluates both the ablated backbone and the same backbone
augmented with the continuous temporal adapter.  This supports a direct
VQ/no-VQ x adapter/no-adapter factorial comparison and additional structural
ablations.  Data access, training, and evaluation are single-process and
single-threaded.
"""
...
from dynafuse_core import MasterTemporalResidual, pearson_loss
...
VARIANT_DESCRIPTIONS = {
    "no_vq": "replace the discrete code with its continuous pre-quantization representation",
    "no_cross_stock": "replace the cross-stock Transformer with an identity map",
    "no_market_prior": "replace the Market63 prior with a zero vector",
    "uniform_expert_gate": "replace the code-conditioned expert gate with equal weights",
    "no_temporal_encoder": "remove the backbone temporal Transformer and use the state token directly",
    "no_contrastive": "remove the codebook contrastive objective from stage-one training",
    "no_reconstruction": "remove the Alpha158 reconstruction objective from stage-one training",
    "no_aux_return": "remove the auxiliary return objective from stage-one training",
}
...
class AblatedPrismSpatial(nn.Module):
    def __init__(self, variant: str, d_model: int = 64, codebook_size: int = 512):
        super().__init__()
        self.variant = variant
        self.use_vq = variant != "no_vq"
        self.use_cross_stock = variant != "no_cross_stock"
        self.use_market_prior = variant != "no_market_prior"
        self.gru = nn.GRU(158, d_model, batch_first=True)
        layer = nn.TransformerEncoderLayer(
            d_model, 2, d_model * 2, 0.1, batch_first=True, norm_first=True
        )
        ...
        self.decoder = nn.Sequential(
            nn.Linear(d_model + 64, 256), nn.GELU(), nn.Linear(256, 158)
        )
        self.aux = nn.Sequential(
            nn.Linear(d_model + 64, 128), nn.GELU(), nn.Linear(128, 1)
        )
    ...
    def encode(self, x: torch.Tensor):
        ...
        norm = (stock - mean) / std
        h = self.gru(norm)[1][-1]
        z = self.cross(h.unsqueeze(0)).squeeze(0) if self.use_cross_stock else h
        if self.use_vq:
            distances = (
                z.square().sum(1, keepdim=True)
                + self.codebook.weight.square().sum(1)
                - 2.0 * z @ self.codebook.weight.t()
            )
        ...
        if self.use_market_prior:
            ...
            prior = torch.zeros(z.shape[0], 64, dtype=z.dtype, device=z.device)
...
class AblatedPrismRanker(nn.Module):
    def __init__(self, spatial: AblatedPrismSpatial, variant: str, d_model: int = 64, experts: int = 2):
        super().__init__()
        self.spatial = spatial
        self.variant = variant
        self.use_temporal_encoder = variant != "no_temporal_encoder"
        self.use_conditioned_gate = variant != "uniform_expert_gate"
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_temporal_encoder:
            ...
            context = state
        if self.use_conditioned_gate:
            weights = torch.softmax(self.gate(state), dim=-1)
        else:
            weights = torch.full(
                (state.shape[0], len(self.experts)), 1.0 / len(self.experts),
...
class TAPrismAblation(nn.Module):
    def __init__(self, base: AblatedPrismRanker):
        super().__init__()
        self.base = base
...
def stage1_loss(spatial: AblatedPrismSpatial, variant: str, x: torch.Tensor, target: torch.Tensor):
    z, quant, _, reconstruction, aux = spatial(x)
    terms: dict[str, torch.Tensor] = {}
    if variant != "no_reconstruction":
        terms["reconstruction"] = F.mse_loss(reconstruction, x[:, -1, :158])
    if spatial.use_vq:
        terms["vq"] = F.mse_loss(z.detach(), quant) + 0.25 * F.mse_loss(z, quant.detach())
        if variant != "no_contrastive":
            distances = torch.cdist(
                F.normalize(z, dim=-1), F.normalize(spatial.codebook.weight, dim=-1)
            )
            assignments = distances.argmin(1)
            terms["contrastive"] = F.cross_entropy(-distances / 0.07, assignments)
    if variant != "no_aux_return":
        terms["aux_return"] = 1e-4 * F.mse_loss(aux, target)
    if not terms:
        raise ValueError(f"variant {variant} removed every stage-one objective")
...
def train_spatial(spatial, variant, store, dates, optimizer, device, epochs, max_train_days):
    ...
    for epoch in range(epochs):
        ...
        history.append({
            ...
            "terms": {name: float(np.mean(values)) for name, values in term_sums.items()},
        })
...
def load_full_spatial(spatial: AblatedPrismSpatial, checkpoint: Path) -> None:
    state = torch.load(checkpoint, map_location="cpu", weights_only=True)
    spatial_state = {
        name.removeprefix("spatial."): value
        for name, value in state.items() if name.startswith("spatial.")
    }
    missing, unexpected = spatial.load_state_dict(spatial_state, strict=False)
    if missing or unexpected:
        raise RuntimeError(f"incompatible spatial checkpoint; missing={missing}, unexpected={unexpected}")
    for parameter in spatial.parameters():
        parameter.requires_grad_(False)
...
def main() -> None:
    ...
    parser.add_argument("--full-base-dir", type=Path, default=ROOT / "results" / "recent_model_screening")
    parser.add_argument("--output-dir", type=Path, default=ROOT / "results" / "prism_backbone_ablation")
    ...
    spatial = AblatedPrismSpatial(args.variant).to(device)
    can_reuse_spatial = args.variant in {"uniform_expert_gate", "no_temporal_encoder"}
    if can_reuse_spatial:
        full_checkpoint = args.full_base_dir / f"prism_vq_{args.universe}_seed{args.seed}_best.pt"
        load_full_spatial(spatial, full_checkpoint)
        spatial_history = []
        spatial_source = str(full_checkpoint.resolve())
    else:
        optimizer_spatial = torch.optim.AdamW(spatial.parameters(), lr=args.lr, weight_decay=1e-4)
        spatial_history = train_spatial(
            spatial, args.variant, store, train_dates.copy(), optimizer_spatial,
            device, args.stage1_epochs, args.max_train_days,
        )
        spatial_source = "retrained_from_scratch"

    base = AblatedPrismRanker(spatial, args.variant).to(device)
    base_trainable = [p for p in base.parameters() if p.requires_grad]
    optimizer_base = torch.optim.AdamW(base_trainable, lr=args.lr, weight_decay=1e-4)
    base_history, base_best_epoch, base_state = train_predictor(
        base, f"{args.variant}/base", store, train_dates.copy(), optimizer_base,
        device, args.base_epochs, args.patience, args.seed,
        args.max_train_days, args.max_eval_days,
    )
    ...
    adapter = TAPrismAblation(base).to(device)
    adapter_trainable = [p for p in adapter.parameters() if p.requires_grad]
    optimizer_adapter = torch.optim.AdamW(adapter_trainable, lr=args.lr, weight_decay=1e-4)
    adapter_history, adapter_best_epoch, adapter_state = train_predictor(
        adapter, f"{args.variant}/adapter", store, train_dates.copy(), optimizer_adapter,
        device, args.adapter_epochs, args.patience, args.seed,
        args.max_train_days, args.max_eval_days,
    )
    torch.save(adapter_state, args.output_dir / f"{prefix}_adapter_best.pt")
    adapter_rows, adapter_predictions = evaluate(
        adapter, store, TEST_START, TEST_END, device, args.max_eval_days
    )
    ...
    result = {
        "experiment": "PRISM-VQ backbone structural ablation with and without TA adapter",
        "variant": args.variant,
        "what_changed": VARIANT_DESCRIPTIONS[args.variant],
        ...
        "base": {
            ...
            "metrics": metrics_by_period(base_rows),
            "parameters": sum(p.numel() for p in base.parameters()),
            "trainable_parameters_stage2": len(base_state) and sum(p.numel() for p in base_trainable),
            "checkpoint": str(base_checkpoint.resolve()),
        },
        "with_temporal_adapter": {
            "best_epoch": adapter_best_epoch,
            "history": adapter_history,
            "metrics": metrics_by_period(adapter_rows),
            "total_parameters": sum(p.numel() for p in adapter.parameters()),
            "adapter_trainable_parameters": sum(p.numel() for p in adapter_trainable),
            "final_residual_scale": float(torch.tanh(adapter.residual_scale).detach().cpu()),
        },
        "elapsed_seconds": time.time() - started,
        "mode": "single-process single-threaded",
        "config": vars(args) | {
            "full_base_dir": str(args.full_base_dir.resolve()),
    ...
    log(json.dumps({
        ...
        "with_temporal_adapter": result["with_temporal_adapter"]["metrics"],

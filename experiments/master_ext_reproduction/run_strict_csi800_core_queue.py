"""Resume-safe serial queue for the strict CSI 800 DynaFuse reproduction."""
...
RESULTS = ROOT / "results" / "protocol_strict_norm_20191224_csi800"
STATUS = RESULTS / "strict_csi800_core_queue_status.json"
...
def main() -> None:
    ...
    tasks = [
        ...
        (
            "master_seed0",
            master_dir / "master_full_csi800_seed0.json",
            protocol_command(
                "run_master_component_ablation", "--variant", "full",
                "--universe", "csi800", "--seed", "0", "--epochs", "40",
                "--patience", "40", "--output-dir", str(master_dir),
            ),
        ),
        (
            "continuous_ta_seed0",
            continuous_dir / "no_vq_csi800_seed0.json",
            protocol_command(
                "run_prism_backbone_ablation", "--variant", "no_vq",
                "--universe", "csi800", "--seed", "0", "--stage1-epochs", "4",
        ...
        (
            "top1_sparse_seed0",
            sparse_dir / "ta_deformable_topk1_csi800_seed0.json",
            protocol_command(
                "run_topvenue_ta_components_p2", "--variant", "deformable",
                "--selected", "1", "--universe", "csi800", "--seed", "0",
        ...
        (
            ...
            final_dir / "strict_dynafuse_csi800_seed0.json",
            protocol_command(
                "analyze_strict_dynafuse", "--protocol-root", str(RESULTS),
                "--output-dir", str(final_dir), "--universe", "csi800",
            ),
        ),
    ]
    ...
    final = json.loads((final_dir / "strict_dynafuse_csi800_seed0.json").read_text(encoding="utf-8"))
    write_status({
